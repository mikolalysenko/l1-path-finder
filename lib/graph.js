...
function heuristic(tdist, tx, ty, node) {
  ...
  for(var i=0; i<NUM_LANDMARKS; ++i) {
    pi = Math.max(pi, Math.abs(tdist[i]-ndist[i]))
  }
...
proto.addS = function(v) {
  if((v.state & 2) === 0) {
    v.weight      = Math.abs(this.srcX - v.x) + Math.abs(this.srcY - v.y) + heuristic(this.landmarkDist, this.dstX, this.dstY, v)
    v.state       |= 2
    v.pred        = null
...
proto.search = function() {
  ...
  if( this.lastS && this.lastT &&
    ...
    for(var toVisit=this.toVisit; toVisit!==NIL; ) {
      ...
      var nx  = +node.x
      var ny  = +node.y
      var nodeH = heuristic(tdist, tx, ty, node)
      var d = Math.floor(node.weight - nodeH)
      ...
      for(var i=0; i<n; ++i) {
        ...
        var vx = +v.x
        var vy = +v.y
        var w = d + Math.abs(nx-vx) + Math.abs(ny-vy) + heuristic(tdist, tx, ty, v)
        if(state < 2) {
          v.state    |= 2
          v.weight   = w
          v.pred     = node
