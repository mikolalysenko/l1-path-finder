...
proto.search = function(tx, ty, sx, sy, out) {
  ...
  while(node) {
    ...
    if(idx < 0) {
      graph.addS(node.verts[0])
    } else if(node.verts[idx].y === sy) {
      graph.addS(node.verts[idx])
    } else {
      graph.addS(node.verts[idx])
      if(idx < node.verts.length - 1) {
        graph.addS(node.verts[idx+1])
      }
    }
    if(node.x < sx) {
      node = node.left
  ...
  while(node) {
    ...
    if(idx < 0) {
      graph.addT(node.verts[0])
    } else if(node.verts[idx].y === ty) {
      graph.addT(node.verts[idx])
    } else {
      graph.addT(node.verts[idx])
      if(idx < node.verts.length - 1) {
        graph.addT(node.verts[idx+1])
      }
    }
    if(node.x < tx) {
      node = node.left
